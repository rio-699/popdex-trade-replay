...
function normalizeOrder(o) {
  ...
  return {
    ...
    orderType: o.orderType ?? o.type ?? null,
    side: (o.side ?? '').toString().toUpperCase() || null,
    price: num(o.price),
    triggerPrice: num(o.triggerPrice ?? o.stopPrice),
    quantity: num(o.quantity ?? o.qty ?? o.size),
    timestamp: num(o.createdTime ?? o.timestamp ?? o.time),
...
function buildTimeline(trade) {
  ...
  for (const o of trade.orders) {
    if (o.timestamp != null) {
      events.push({ type: o.status === 'FILLED' ? 'Order filled' : 'Order created', timestamp: o.timestamp, data: o });
    }
  }
