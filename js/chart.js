...
class ReplayChart {
   ...
   *  - Dataset fits comfortably at a readable width: pad a small,
   *    data-proportional margin on both sides so the candles fill most of
   *    the frame without touching the edges, centered so a middle-crop (the
   *    phone clip) or a quick glance always lands on real data.
   *  - Dataset is larger than that: show a centered window of just
   *    `desiredBars` candles around the entry (TradingView's usual
   *    "recent window" default) instead of shrinking every candle to fit —
   *    normal zoom/scroll still reaches the rest. The window gets shifted
   *    to stay inside the actual data bounds when entry sits near either
   *    edge of the fetched range, instead of padding out into blank space.
   */
  fitToFullRange(centerTsMs) {
    ...
    if (dataLen <= desiredBars) {
      // Fewer real candles than the box could show thinly. Pad a modest,
      // data-proportional margin on both sides instead of stretching to the
      // full box width, so the candles keep a natural size, fill most of
      // the frame, and sit centered rather than stranded near an edge.
      const MIN_PADDING_BARS = 4;
      const paddingBars = Math.max(MIN_PADDING_BARS, Math.round(dataLen * 0.12));
      this.chart.timeScale().setVisibleLogicalRange({ from: -paddingBars, to: dataLen - 1 + paddingBars });
      return;
    }

    // More real candles than the box can show at a readable width. Center a
    // desiredBars-wide window on the entry candle rather than cramming the
    // whole dataset in — the rest is a scroll/zoom-out away.
