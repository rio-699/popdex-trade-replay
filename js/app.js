...
const App = {
  ...
  renderTradeRow(trade) {
    ...
    row.innerHTML = `
      ...
      <div class="trade-row__cell ${pnl > 0 ? 'positive' : pnl < 0 ? 'negative' : ''}">${p.realizedPnl != null ? formatSignedCurrency(p.realizedPnl) : '\u2014'}</div>
  ...
  bindReplayControls() {
    ...
    $('#speed-custom').addEventListener('change', (e) => {
      const v = Math.min(100, Math.max(0.1, Number(e.target.value) || 1));
      $$('.speed-btn').forEach((b) => b.classList.remove('is-active'));
  ...
  renderReplaySkeleton(trade) {
    ...
    $$('.speed-btn').forEach((b) => b.classList.toggle('is-active', b.dataset.speed === '1'));
    $('#speed-custom').value = '';
    ReplayStateInstance.setSpeed(1);
  ...
  async loadReplayData(trade, interval) {
    ...
    // Small amount of surrounding context so entry/exit aren't flush against the chart edges (§45).
    const windowMs = (p.closeTimestamp ?? p.openTimestamp) - p.openTimestamp;
    const pad = Math.max(60_000, Math.round(windowMs * 0.05));
    ...
    const [orders, fills, candlesRaw, funding, liquidations] = await Promise.all([
      softLoad(() => PopDexApi.fetchOrders(walletId, {
  ...
  async downloadClip(format = 'desktop') {
    ...
    try {
      ...
      for (let i = 0; i <= RENDER_SAMPLES; i++) {
        ...
        const frame = await html2canvas(target, { backgroundColor: '#0a0c10', scale, useCORS: true, logging: false });
