...
const PopDexApi = {
  ...
  /**
   * Pages through the FULL order history for a wallet/window. Unlike
   * fetchAllFills/fetchAllHistoricalPositions/fetchAllAccountFunding, this
   * endpoint's `cursor` is a page NUMBER (docs: "Default: 1"), and the
   * response envelope only echoes back the cursor it was given plus a
   * `total` record count — it never hands back a `nextPageCursor`/
   * `nextCursor` forward-pointing token the way the other history
   * endpoints do. So this pager advances `cursor` by incrementing the page
   * index itself and stops once it has collected `total` records (or a
   * page comes back short/empty), instead of looking for a "next" field
   * that this endpoint doesn't return.
   *
   * Plain fetchOrders only ever returns page 1 (default 20 orders, cap
   * 100) — fine for a quiet trade, but silently truncates the order list
   * for any position with more orders than that (e.g. a scaled-in/out
   * entry), which is one direct way "order history" can look thinner than
   * what the position's PnL was actually built from.
   *
   * `orderId` is the real, always-unique id the live API returns for every
   * order (see normalizeOrder in core.js), so it's used directly as the
   * dedupe key instead of a composite fallback.
   */
  async fetchAllOrders(walletId, {
    startTime, endTime, symbol, category, orderType, side, sourceType,
    clientOid, includeUnfilled, pageSize = 100, maxPages = 5000, maxTotal = 1_000_000,
  } = {}) {
    const all = [];
    const seen = new Set();
    let cursor = 1;
    ...
    for (let page = 0; page < maxPages; page++) {
      const res = await request(
        `/api/v1/account/${encodeURIComponent(walletId)}/history/orders`,
        {
          startTime, endTime, symbol, category, orderType, side, sourceType,
          clientOid, includeUnfilled, cursor, limit: pageSize,
        },
        { cache: false, envelope: true },
      );
      const list = Array.isArray(res?.data) ? res.data : (res?.data?.list || res?.data?.items || []);
      ...
      for (const item of list) {
        const key = item.orderId ?? item.id
          ?? `${item.createdAt ?? item.timestamp}:${item.symbol}:${item.side}:${item.qty}`;
        if (seen.has(key)) continue;
        seen.add(key);
        all.push(item);
      }
      ...
      if (list.length < pageSize) break; // short page — nothing left to fetch
      cursor += 1;
    }
    return all;
  },
