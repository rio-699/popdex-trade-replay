...
const DemoData = {
  ...
  enrichTrade(trade) {
    ...
    const barMs = durationMs / candleCount;
    const noiseScale = (Math.abs(p.exitPrice - p.entryPrice) / candleCount) * 6 || p.entryPrice * 0.0005;

    const candles = [];
    let price = p.entryPrice;
    const drift = (p.exitPrice - p.entryPrice) / candleCount;
    for (let i = 0; i < candleCount; i++) {
      const t = p.openTimestamp + i * barMs;
    ...
    // Pad candles before entry and after exit so the entry candle has real
    // history on both sides — without this it was the very first bar in the
    // dataset, which forced it flush against the left edge of the chart no
    // matter what the initial-view logic tried to do (see
    // ReplayChart.fitToFullRange). Mirrors the pre/post padding the real API
    // path fetches around a trade.
    const padCount = Math.max(150, Math.round(candleCount * 0.6));
    const prePad = [];
    let prePrice = p.entryPrice;
    for (let i = 1; i <= padCount; i++) {
      const t = p.openTimestamp - i * barMs;
      const noise = (rnd() - 0.5) * noiseScale;
      const close = prePrice;
      const open = close - noise;
      const high = Math.max(open, close) + Math.abs(noise) * 0.5;
      const low = Math.min(open, close) - Math.abs(noise) * 0.5;
      prePad.push(normalizeCandle([t, open, high, low, close, rnd() * 5, rnd() * 5 * close]));
      prePrice = open;
    }
    prePad.reverse();

    const postPad = [];
    let postPrice = p.exitPrice;
    for (let i = 1; i <= padCount; i++) {
      const t = p.closeTimestamp + i * barMs;
      const noise = (rnd() - 0.5) * noiseScale;
      const open = postPrice;
      const close = open + noise;
      const high = Math.max(open, close) + Math.abs(noise) * 0.5;
      const low = Math.min(open, close) - Math.abs(noise) * 0.5;
      postPad.push(normalizeCandle([t, open, high, low, close, rnd() * 5, rnd() * 5 * close]));
      postPrice = close;
    }

    candles.unshift(...prePad);
    candles.push(...postPad);
